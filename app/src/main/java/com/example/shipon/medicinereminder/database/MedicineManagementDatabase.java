...
public class MedicineManagementDatabase {
    ...
    public ArrayList<MedicinePerRow> retrieveMedicineByName(String name) {
        MedicineRow.clear();
        SQLiteDatabase sqLiteDatabase = databaseHelper.getReadableDatabase();
        Cursor cursor = sqLiteDatabase.rawQuery("SELECT * FROM " + DatabaseHelper.TABLE_MEDICINE_DATE_TIME + " WHERE " + databaseHelper.MEDICINE_NAME_TABLE2 + " =? ", new String[]{name});
        if (cursor.moveToFirst()) {
            do {
